...
int	GetLD_MARSTATE(int *x) { return(x[LD_MARSTATE]);}
int	GetVAB(int *x) { return(x[VAB]);}
...
void rdump(FILE * dumpsim_file) {                               
	...
	/*Debug purposes*/
	    printf("STATE_NUMBER integer : %d\n\n", CURRENT_LATCHES.STATE_NUMBER);
		 printf("MARSTATE : %d\n\n", CURRENT_LATCHES.MARSTATE);
		 printf("VA Register:  0x%0.4x\n\n", CURRENT_LATCHES.VA);
		 printf("PTBR Register:  0x%0.4x\n\n", CURRENT_LATCHES.PTBR);
		  printf("interrupt bit: %d", CURRENT_LATCHES.INT);
...
/*Virtual Memory related gate stuff*/
#define G_PTBR_YES 9
#define G_VA_YES 10
#define VIRTUAL_STATE 55

#define EXCEPTION_STATE 41
#define EXC_ILLOP 0x05
#define INT_TIMER 0x01
#define EXC_PROT 0x04
#define EXC_UNALIGN 0x03
#define EXC_PAGEFAULT 0x02
#define shouldInterrupt 300

/*More virtual memory stuff here. Mainly masks*/
#define PAGE_NUMBER_MASK 0xFE00
#define PTBR_MASK 0xFF00
#define PTE_REFERENCE_MASK 0x0001
#define PTE_MODIFY_MASK	0x0002
#define PTE_PROTECTION_MASK 0x0008
/*Apparently this was already defined #define PTE_PFN_MASK 0x3E00 
#define PTE_VALID_MASK 0x0004*/
#define VA_MASK 0x01FF
...
int PTBROutput;
int VAOutput;
int endofCycleMemPROT;
int endofCycleMemFAULT;
...
int * instruc = CURRENT_LATCHES.MICROINSTRUCTION;
	...
	int ldMARSTATE = GetLD_MARSTATE(instruc);
	int VAB = GetVAB(instruc);	
	...
	if(CURRENT_LATCHES.STATE_NUMBER == 12){
		printf("This should generate a protection exception soon in jump");
	}
	
	if(CURRENT_LATCHES.STATE_NUMBER == 8){
		printf("This is probably the end of the ISR");
	}
	...
	if((CURRENT_LATCHES.INT == 1) && ((currentState>>1) == 9)){
	jState = EXCEPTION_STATE;
	...
	if(CURRENT_LATCHES.STATE_NUMBER == 62){
		jState = CURRENT_LATCHES.MARSTATE;
	}
	
	if(ldMARSTATE && VAB){
	/*Know we are in one of the exception handling states need to load MARSTATE and branch to appropriate either exception state or virtual chain*/
	jState = VIRTUAL_STATE;
	}
...
void eval_bus_drivers() {
   ...
   */    
	...
	int G_PTBR = GetGATE_PTBR(instruc);
	int G_VA = GetGATE_VA(instruc);
...
/*This logica determines SR2 based on immediate bit and SR1 based on SR1MUX output*/	
	...
	int VAB = GetVAB(instruc);
	if(VAB == 0){
	/*Virtual Memory Stuff: Need to make slight adjustment for putting physical address on bus*/
	G_YES = G_MDR_YES;
	}
	...
	MDROutput = Low16bits(memContent);
		...
		if(VAB == 1){
		/*Need to just output PFN here*/
		MDROutput = ((CURRENT_LATCHES.MDR)&(PTE_PFN_MASK)); 
...
/**/
	...
	if(G_PTBR){
		...
		/*GATE PTBR needs to output the following Load the MAR with the address of the PTE of the page containing the VA:
		MAR[15:8] gets PTBR[15:8]
		MAR[7:0] gets LSHF(VA[15:9], 1)
		MAR gets PTBR + (2 × page_number)*/
		int pageNumber = (CURRENT_LATCHES.VA)&(PAGE_NUMBER_MASK);
		int PTBR = (CURRENT_LATCHES.PTBR)&(PTBR_MASK);
		
		G_YES = G_PTBR_YES;
		pageNumber = pageNumber>>8; /*Check this but I think this effectively is 2xPage number which we need*/
		
		PTBROutput = PTBR + pageNumber;
	
	}
	
	if(G_VA){
	/*Virtual Memory Stuff: At this stage you are definitely in state 62. Need to construct PA (Physical Address). Part comes from Gate_Va. VA[8:0]*/
	G_YES = G_VA_YES;
	VAOutput = (CURRENT_LATCHES.VA &VA_MASK);
	}
...
void latch_datapath_values() {
   ...
   */       
	...
	/*Virtual Memory related stuff*/
	int ldMARSTATE = GetLD_MARSTATE(instruc);
	int ldVA = GetLD_VA(instruc);
	int VAB = GetVAB(instruc);
	int J = GetJ(instruc);
	...
	/*Need to get rid of this now because protection exception detected differently*/
	/*
	...
	/*Virtual stuff*/
	/*Warning: May mess up because state 57 is memory access. So CHECK THIS*/
	if(CURRENT_LATCHES.STATE_NUMBER == 59){
	/*Calculate protection exception here. Based on PTE P bit and supervisor mode
	Also Calculate page fault exception here. Based on whether V bit is set or not in PTE*/
	int priviledge = Low16bits((CURRENT_LATCHES.PSR&0x8000)>>15);
	
	/*Protection exception occurs when PTE contains a 0 for protection bit and not a trap instruction and not in supervisor mode*/
	if((!((NEXT_LATCHES.MDR) & (PTE_PROTECTION_MASK)))){
		if((priviledge) &&(CURRENT_LATCHES.MARSTATE != 28)){
				PROT = 1;
				NEXT_LATCHES.EXCV = EXC_PROT;
				NEXT_LATCHES.E = 1;
				printf("Protection Exception Detected\n");
	...
	if((!((NEXT_LATCHES.MDR) & (PTE_VALID_MASK)))){
		
				PAGEFAULT = 1;
				NEXT_LATCHES.EXCV = EXC_PAGEFAULT;
				NEXT_LATCHES.E = 1;
	...
	if(VAB){
	/*Virtual Memory Stuff: We know that the MDR contains the PTE Need to modify and calculate Reference and modify bits in PTE */
	NEXT_LATCHES.MDR = ((CURRENT_LATCHES.MDR)|(PTE_REFERENCE_MASK));
	/*
	printf("\n DEbug: In virtual modify PTE state, current_latches.MDR is : 0x%0.4x\n", CURRENT_LATCHES.MDR);
	printf("current_latches.MDR is : 0x%0.4x\n", PTE_REFERENCE_MASK);
	printf("current_latches.MDR is : 0x%0.4x\n", NEXT_LATCHES.MDR);
	*/
	if(CURRENT_LATCHES.MARSTATE == 23 || CURRENT_LATCHES.MARSTATE == 34){
			/*Do this because these MARSTATES are associated with stb and stw the two write situations*/
			/*printf("Debug we think the next MARSTATE is a write so either 23 or 24");*/
			NEXT_LATCHES.MDR = ((CURRENT_LATCHES.MDR)|(PTE_MODIFY_MASK));
	}
...
/*Update PSR as well. Note that PSR contains condition codes in register for both user and supervisor mode*/
	...
	/*Virtual Memory ld stuff*/
	if(ldVA){
		NEXT_LATCHES.VA = CURRENT_LATCHES.MAR;
		/*Don't think BUS is necessary here, can just use a direct connection*/
	}
	
	if(ldMARSTATE && VAB){
	/*Know we are in one of the exception handling states need to load MARSTATE and branch to appropriate either exception state or virtual chain*/
	
	NEXT_LATCHES.MARSTATE = J;
